...
{
    public class MDExplorationBasinQuery : BaseCrudQuery
    {
        public override string SelectPagedQuery => @"
            select  ba.BasinID,
                ba.BasinName
            from dbo.MD_ExplorationBasin ba";
        ...
        public override string CountQuery => @"
            select count(1) from dbo.MD_ExplorationBasin ba";

        public override string LookupTextQuery => @"
            select ba.BasinName 
            from dbo.MD_ExplorationBasin ba
            where ba.BasinID = {0}";
        public override string GenerateID => @"SELECT COUNT(*) FROM dbo.MD_ExplorationBasin ba";
