...
{
    public class MDExplorationAreaQuery : BaseCrudQuery
    {
        public override string SelectPagedQuery => @"
            select  a.xAreaID,
                a.xAreaName
            from dbo.MD_ExplorationArea a";
        ...
        public override string CountQuery => @"
            select count(1) from dbo.MD_ExplorationArea a";

        public override string LookupTextQuery => @"
            select a.xAreaName 
            from dbo.MD_ExplorationArea a
            where a.xAssetID = '{0}'";
        public override string GenerateID => @"SELECT COUNT(*) FROM dbo.MD_ExplorationArea a";
