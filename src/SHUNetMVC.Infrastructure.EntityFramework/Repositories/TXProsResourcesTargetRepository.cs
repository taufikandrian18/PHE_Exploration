...
{
    public class TXProsResourcesTargetRepository : BaseCrudRepository<TX_ProsResourcesTarget, TXProsResourcesTargetDto, TXPropectiveResourceTargetWthFields, TXProsResourcesTargetQuery>, ITXProsResourcesTargetRepository
    {
        public TXProsResourcesTargetRepository(DB_PHE_ExplorationEntities explorationContext, IConnectionProvider connection, DB_PHE_HRIS_DEVEntities hrContext)
            : base(explorationContext, connection, new TXProsResourcesTargetQuery(), hrContext)
        {

        }

        public override async Task Create(TXProsResourcesTargetDto model)
        {
            var entity = model.ToEntity();
            try
            {
                _explorationContext.TX_ProsResourcesTarget.Add(entity);
        ...
        {
            List<TX_ProsResourcesTarget> result = _explorationContext.Set<TX_ProsResourcesTarget>().ToList();
            List<TXProsResourcesTargetDto> dto = (List<TXProsResourcesTargetDto>)Activator.CreateInstance(typeof(List<TXProsResourcesTargetDto>), result);
            return dto;
        }

        public async Task<IEnumerable<TXProsResourcesTargetExcelDto>> GetExportProsResourceTargetByStructureID(string structureID)
        {
            TXProsResourcesTargetQuery queryObj = new TXProsResourcesTargetQuery();
            using (var connection = OpenConnection())
            {
                var querySQL = string.Format(queryObj.ExcelExportQuery, structureID);
                var lookupText = await connection.QueryAsync<TXProsResourcesTargetExcelDto>(querySQL);
                return lookupText;
            }
        }
    }
